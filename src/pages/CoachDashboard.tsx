/**
 * CoachDashboard.tsx — Visão do Coach (com Anamnese + RoutineBuilder)
 */
...
function PlanEditorDialog({ student, onClose }: { student: StudentStatus; onClose: () => void }) {
  ...
  return (
    <DialogContent className="max-w-sm">
      <DialogHeader>
        <DialogTitle className="text-base font-bold">Editar Plano — {student.name}</DialogTitle>
      </DialogHeader>
      <div className="space-y-4 pt-1">
        <div className="grid grid-cols-2 gap-3">
          <div>
            <Label className="text-xs text-muted-foreground">Calorias/dia</Label>
            <Input value={calories} onChange={(e) => setCalories(e.target.value)} className="mt-1 h-9 text-sm" type="number" />
          </div>
          <div>
            <Label className="text-xs text-muted-foreground">Proteína (g)</Label>
            <Input value={protein} onChange={(e) => setProtein(e.target.value)} className="mt-1 h-9 text-sm" type="number" />
          </div>
        </div>
        <div>
          <Label className="text-xs text-muted-foreground">Observação</Label>
          <Textarea value={note} onChange={(e) => setNote(e.target.value)} placeholder="Ex: Aumentar proteína pós-treino..." className="mt-1 text-sm resize-none h-20" />
        </div>
...
function StudentRow({
  ...
  return (
    <div className={`flex items-center gap-4 px-4 py-3.5 rounded-xl border transition-colors ${
      student.alertLevel === "critical" ? "bg-red-50/60 border-red-100" :
      student.alertLevel === "warning" ? "bg-amber-50/50 border-amber-100" :
      "bg-card border-border"
    }`}>
      <div className="w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold shrink-0"
        style={{
          background: student.alertLevel === "critical" ? "#FEE2E2" : student.alertLevel === "warning" ? "#FEF9C3" : "#DBEAFE",
          color: student.alertLevel === "critical" ? "#DC2626" : student.alertLevel === "warning" ? "#D97706" : "#2563EB",
        }}>
      ...
      {student.currentWeight && student.targetWeight && (
        <div className="hidden sm:block text-right shrink-0">
          ...
          <p className="text-sm font-semibold text-foreground">
            {student.currentWeight}<span className="text-muted-foreground"> / </span><span className="text-primary">{student.targetWeight}</span> kg
          </p>
        </div>
      )}
...
export default function CoachDashboard() {
  ...
  // Detail views (Anamnesis or Routine)
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-4xl mx-auto px-4 py-6 space-y-6">
        ...
        {stats.critical > 0 && (
          <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-red-800">{stats.critical} aluno{stats.critical > 1 ? "s" : ""} sem atividade há 3+ dias</p>
              <p className="text-xs text-red-600 mt-0.5">Clique nos ícones para ver anamnese, criar rotina ou editar plano.</p>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-3 gap-3 pt-2">
          {[
            ...
            <button key={label} className="flex flex-col items-center gap-2 py-4 rounded-xl border border-border bg-card hover:border-primary/30 hover:bg-accent/40 transition-colors">
      ...
      <Dialog open={!!editingStudent} onOpenChange={() => setEditingStudent(null)}>
        {editingStudent && <PlanEditorDialog student={editingStudent} onClose={() => setEditingStudent(null)} />}
      </Dialog>
    </div>
  );
}
